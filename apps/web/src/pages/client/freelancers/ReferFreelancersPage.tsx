...
export const ReferFreelancersPage: React.FC = () => {
  const { referrals, createReferral } = useApp();
  ...
  const handleSubmit = (event: React.FormEvent) => {
    ...
    createReferral({ name, email, specialty, note });
    resetForm();
    setStatusMessage('Referral saved and invite marked as sent.');
    ...
    <FreelancersLayout activeItem="Bring freelancers to Marché">
      <div className="space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
          <div>
            <h1 className="text-2xl font-extrabold text-ink tracking-tight">Bring freelancers to Marché</h1>
        ...
        <Card className="p-6 space-y-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-ink-muted mt-0.5">The invite is tracked locally in this preview.</p>
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <Input value={specialty} onChange={(event) => setSpecialty(event.target.value)} placeholder="Photography, catering, event planning..." />
            </div>

            <div>
              <label className="block text-xs font-semibold text-ink mb-1">Personal note</label>
              <Textarea rows={4} value={note} onChange={(event) => setNote(event.target.value)} placeholder="Optional message for the provider" />
            </div>
        ...
        <Card className="p-6 space-y-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-ink-muted mt-0.5">Providers you have invited to join Marché.</p>
            </div>
          </div>
              ...
              {referrals.map((referral) => (
                <div key={referral.id} className="py-4 flex items-start justify-between gap-4">
                  <div className="min-w-0">
                    ...
                    {referral.note && <p className="text-xs text-ink-muted mt-1 line-clamp-2">{referral.note}</p>}
                  </div>
