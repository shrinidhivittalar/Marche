...
// Empty/missing is the only signal this needs: an unset env var, not a
// specific invalid value, so the button hides itself rather than rendering
// something that can only ever fail (see AuthSignInPage's early check
// below).
...
export const AuthSignInPage: React.FC = () => {
  const { navigate, loginWithCredentials, loginWithGoogle, requestPasswordReset } = useApp();
  ...
  const handleGoogleSignIn = () => {
    if (!GOOGLE_CLIENT_ID) return; // button is hidden in this case; defensive only
    setErrorMessage(null);
    setInfoMessage(null);
    setIsGoogleSubmitting(true);
    triggerGoogleSignIn(GOOGLE_CLIENT_ID, async (idToken) => {
      try {
        const { verificationEmailSent } = await loginWithGoogle(idToken);
        if (verificationEmailSent) {
          setInfoMessage(
            'Google could not confirm this email address. Check your inbox for a verification link, then sign in.',
          );
        }
        // On success, loginWithGoogle already navigated away — nothing left to do here.
      } catch (err) {
        setErrorMessage(
          err instanceof ApiError ? err.message : 'Something went wrong. Please try again.',
        );
            ...
            {GOOGLE_CLIENT_ID && (
              <>
                ...
                <button
                  ...
                  className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-bg border border-border hover:bg-zinc-100 rounded-xl text-xs font-semibold text-ink transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                >
...
export const AuthSignUpPage: React.FC = () => {
  const { navigate, acceptLegalTerms, registerAccount, loginWithGoogle } = useApp();
  ...
  const handleGoogleSignUp = () => {
    if (!GOOGLE_CLIENT_ID || !agreeTerms) return; // button is disabled/hidden in these cases; defensive only
    setErrorMessage(null);
    setInfoMessage(null);
    setIsGoogleSubmitting(true);
    triggerGoogleSignIn(GOOGLE_CLIENT_ID, async (idToken) => {
      try {
        const { verificationEmailSent } = await loginWithGoogle(idToken);
        acceptLegalTerms({ role, context: 'signup', email });
        if (verificationEmailSent) {
          setInfoMessage(
            'Google could not confirm this email address. Check your inbox for a verification link, then sign in.',
          );
            ...
            {GOOGLE_CLIENT_ID && (
              <>
                ...
                <button
                  ...
                  disabled={isGoogleSubmitting || !agreeTerms}
                  title={!agreeTerms ? 'Agree to the Terms and Privacy Policy first' : undefined}
                  className="w-full flex items-center justify-center gap-2 px-3 py-2 bg-bg border border-border hover:bg-zinc-100 rounded-xl text-xs font-semibold text-ink transition-all cursor-pointer disabled:opacity-60 disabled:cursor-not-allowed"
                >
                  <svg className="w-3.5 h-3.5" viewBox="0 0 24 24">
                    <path
