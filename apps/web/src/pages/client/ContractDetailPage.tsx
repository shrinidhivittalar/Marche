...
export const ContractDetailPage: React.FC<ContractDetailPageProps> = ({ id }) => {
  ...
  // Same reasoning as the payment card on ProposalDetailPage: the webhook is
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8">
      ...
      {/* Payment */}
      <Card className="p-8 space-y-4" data-testid="payment-card">
        <h3 className="text-base font-bold text-ink">Payment</h3>

        {paid ? (
          <p className="text-xs font-semibold text-primary flex items-center gap-2">
            <CheckCircle2 className="w-4 h-4" />₹{amount.toLocaleString('en-IN')} paid for this
            booking.
          </p>
        ) : isClient ? (
          <>
          ...
          <p className="text-xs text-ink-muted">Not yet paid by the client.</p>
        )}
      </Card>
      ...
      <Card className="p-8 space-y-5">
        ...
        {c.status === 'ACTIVE' && (
          <div className="p-4 bg-surface-subtle border border-border text-ink rounded-2xl space-y-2 text-xs">
