...
- [x] **Payments/finances frontend ledger** - Done; finance pages now render computed summaries, transactions, budgets, and provider earnings from `AppContext` contracts/jobs instead of blurred Coming Soon previews.
- [ ] **Real payments/escrow** - Still missing; requires backend accounts, payment-provider integration, webhooks, and server-side ledger records.
- [ ] **Backend/accounts** - None; everything lives in browser localStorage, with no server and no real login/password.
- [x] **Reviews/ratings** - Done in the frontend; clients can review closed contracts, reviews persist in `AppContext`/localStorage, and provider profile/search reputation includes submitted reviews.
- [x] **Settings** - Done in the frontend; client notification toggles are controlled by `AppContext` and persist to localStorage.
- [x] **Public profiles** - Done in the frontend; provider public profile, talent search, and saved talent now use the edited vendor profile overlay from `AppContext`/localStorage instead of only frozen mock data.
- [x] **Saved talent** - Done; save/unsave is wired through `AppContext`, persists to localStorage, and the Saved talent page renders saved providers.
- [x] **Refer flow** - Done in the frontend; clients can submit freelancer referrals, see referral history, and the records persist to `AppContext`/localStorage.
...
- [x] **Identity verification frontend intake** - Done in the frontend; vendors can submit legal name, document type, document last 4, and address from profile, with pending status persisted in `AppContext`/localStorage.
- [ ] **Real KYC compliance** - Still missing; requires backend identity checks, document upload/storage, proof-of-address verification, provider integration, audit trail, and compliance review before withdrawals.
- [x] **Dispute resolution frontend intake** - Done in the frontend; clients/vendors can raise contract disputes with reason and evidence, records persist in `AppContext`/localStorage, and both the counterparty and demo admin are notified.
- [ ] **Real mediation/arbitration** - Still missing; requires backend evidence storage, staff review workflow, payout holds, settlement decisions, and arbitration/escalation policy.
- [x] **Reputation badges frontend rules** - Done in the frontend; Top Rated Plus, Top Rated, Rising Talent, Fast Response, and Verified Pro badges are computed from rating/review count, jobs completed, responsiveness, and verification, then shown in search, profile, and saved talent.
- [ ] **Production badge telemetry** - Still missing; requires backend completion-rate, responsiveness, dispute, and policy data so badges cannot be gamed or locally edited.
- [x] **Manual work diary / proof-of-work frontend** - Done in the frontend; providers can log contract work dates, hours, summaries, and proof links, with entries persisted in `AppContext`/localStorage and visible on the contract page.
- [ ] **Automatic time tracking / proof capture** - Still missing; requires desktop/mobile tracker, screenshots/activity logs, tamper-resistant storage, and backend review.
- [x] **Legal acceptance / agreement snapshot frontend** - Done in the frontend; signup records accepted terms/privacy version to the selected profile, hire confirmation requires explicit booking-agreement acceptance, and new contracts store/show an agreement snapshot.
- [ ] **Real legal agreements / enforceable ToS** - Still missing; requires backend-authenticated acceptance, immutable timestamps, versioned legal documents, standardized contract/NDA templates, and durable audit storage.
- [ ] **Background/credential checks** - Deeper vetting (licenses, certifications) for certain job categories. Lower priority for an MVP, but standard once real money is involved.
- [x] **Saved talent** - Done in the frontend; save/unsave persists and the Saved talent page renders saved providers.
- [x] **Job alert preferences frontend** - Done in the frontend; providers can turn job alerts on/off, choose matching categories, choose location matching, and new-job notifications respect those saved local preferences.
- [ ] **Backend job alert delivery** - Still missing; requires server-side matching, scheduled/digest delivery, email/push channels, unsubscribe controls, and durable per-user preference storage.
- [ ] **Marketplace health analytics** - Admin has audit logs, but no dashboard tracking overall marketplace health (active users, job fill rate, dispute rate, satisfaction).
- [ ] **Mobile access** - Web-only currently; all major competitors have mobile apps since users check messages/alerts on the go.
...
- **The money pages now have a real frontend ledger.** Finances, budgets, transactions, and provider earnings are computed from local `AppContext` state. This is still not real escrow or payout processing.
...
**Bottom line:** the core marketplace logic is already built — it just needs to be *re-pointed* (swap localStorage calls in `AppContext` for real API calls to a server, add a database, add real accounts) rather than built from scratch. **Real escrow/payment processing still needs to be built from zero**, since the current ledger is frontend-only and has no payment provider, webhook handling, or server-side settlement records.
...
- [ ] **Notification preferences** — Basic client notification toggles now persist in Settings, but channel/frequency controls and backend delivery are still missing.
