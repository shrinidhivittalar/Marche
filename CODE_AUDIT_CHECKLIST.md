...
- [ ] `apps/api/src/notifications/services/notifications.service.ts:127-141` — `markAsRead` does `findById` → `markRead` → `findById` again; two extra round trips per call, unavoidable given `updateMany` doesn't return rows. Minor.
- [ ] `apps/api/src/proposals/services/proposals.service.ts:196-206` — in `accept()`, `proposalAccepted` and `connectionEstablished` are awaited sequentially rather than via `Promise.all`; both write to independent recipients and could run concurrently. Negligible impact.
- [ ] `apps/api/src/jobs/tests/jobs.service.spec.ts` (the `cancel` describe block) — `notificationsService.jobCancelled` is wired into the test harness but never asserted as called. module6.md's "Job cancellation notifies relevant proposers" test case is uncovered at this layer.
- [ ] `apps/api/src/proposals/tests/proposals.service.spec.ts` — `notificationsService` is injected into every test but no `submit`/`withdraw`/`accept`/`reject` test asserts it was called, with what recipient, or what `data`. A regression that silently drops a notification call, or swaps client/provider recipients, would pass every existing test here. **Most significant finding in this batch** — the P0 cases are unit-tested in isolation (`notifications.service.spec.ts`) but the cross-module wiring that actually invokes them is not verified anywhere.
